...
@register.simple_tag
def get_kibru_settings(request: WSGIRequest) -> Dict:
    """
    Get Kibru settings, update any defaults from the request, and return
    """
...
@register.simple_tag
def get_kibru_ui_tweaks() -> Dict:
    """
    Return Kibru ui tweaks
    """
    return get_ui_tweaks()


@register.simple_tag
def get_kibru_version() -> str:
    """
...
@register.simple_tag
def kibru_paginator_number(change_list: ChangeList, i: int) -> SafeText:
    """
...
@register.simple_tag
def kibru_list_filter(cl: ChangeList, spec: ListFilter) -> SafeText:
    """
...
@register.simple_tag
def kibruy_admin_url(value: Union[str, ModelBase], admin_site: str = "admin") -> str:
    """
...
@register.filter
def has_kibru_setting(settings: Dict[str, Any], key: str) -> bool:
...
def get_sections(
    ...
    if hasattr(admin_form.model_admin, "kibru_section_order"):
        fieldsets = order_with_respect_to(
            fieldsets, admin_form.model_admin.kibru_section_order, getter=lambda x: x.name
        )
...
def sidebar_status(request: HttpRequest) -> str:
    ...
    if request.COOKIES.get("kibruy_menu", "") == "closed":
