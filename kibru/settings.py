...
DEFAULT_SETTINGS: Dict[str, Any] = {
    ...
    "site_logo": "vendor/adminlte/img/KibruLogo.jpg",
...
CHANGEFORM_TEMPLATES = {
    "single": "kibru/includes/single.html",
    "carousel": "kibru/includes/carousel.html",
    "collapsible": "kibru/includes/collapsible.html",
    "horizontal_tabs": "kibru/includes/horizontal_tabs.html",
    "vertical_tabs": "kibru/includes/vertical_tabs.html",
}
...
def get_settings() -> Dict:
    kibru_settings = copy.deepcopy(DEFAULT_SETTINGS)
    user_settings = {x: y for x, y in getattr(settings, "KIBRU_SETTINGS", {}).items() if y is not None}
    kibru_settings.update(user_settings)

    # Extract search model configuration from search_model setting
    if kibru_settings["search_model"]:
        if not isinstance(kibru_settings["search_model"], list):
            kibru_settings["search_model"] = [kibru_settings["search_model"]]

        kibru_settings["search_models_parsed"] = []
        for search_model in kibru_settings["search_model"]:
            kibru_search_model = {}
            kibru_search_model["search_url"] = get_admin_url(get_search_model_string(search_model))

            model_meta = get_model_meta(search_model)
            if model_meta:
                kibru_search_model["search_name"] = model_meta.verbose_name_plural.title()
            else:
                kibru_search_model["search_name"] = search_model.split(".")[-1] + "s"
            kibru_settings["search_models_parsed"].append(kibru_search_model)

    # Deal with single strings in hide_apps/hide_models and make sure we lower case 'em
    if type(kibru_settings["hide_apps"]) == str:
        kibru_settings["hide_apps"] = [kibru_settings["hide_apps"]]
    kibru_settings["hide_apps"] = [x.lower() for x in kibru_settings["hide_apps"]]

    if type(kibru_settings["hide_models"]) == str:
        kibru_settings["hide_models"] = [kibru_settings["hide_models"]]
    kibru_settings["hide_models"] = [x.lower() for x in kibru_settings["hide_models"]]

    # Ensure icon model names and classes are lower case
    kibru_settings["icons"] = {x.lower(): y.lower() for x, y in kibru_settings.get("icons", {}).items()}

    # Default the site icon using the site logo
    kibru_settings["site_icon"] = kibru_settings["site_icon"] or kibru_settings["site_logo"]

    # Default the login logo using the site logo
    kibru_settings["login_logo"] = kibru_settings["login_logo"] or kibru_settings["site_logo"]

    # Default the login logo dark using the login logo
    kibru_settings["login_logo_dark"] = kibru_settings["login_logo_dark"] or kibru_settings["login_logo"]

    # ensure all model names are lower cased
    kibru_settings["changeform_format_overrides"] = {
        x.lower(): y.lower() for x, y in kibru_settings.get("changeform_format_overrides", {}).items()
    }

    return kibru_settings


def get_ui_tweaks() -> Dict:
    raw_tweaks = copy.deepcopy(DEFAULT_UI_TWEAKS)
    raw_tweaks.update(getattr(settings, "KIBRU_UI_TWEAKS", {}))
